...
| ~~`errorEnvelopeMiddleware` is dead code~~ | **CLOSED 2026-08-10** | Fixed, not just re-described. `errorMiddleware` was deleted from `shared/http.ts` and unmounted, leaving `errorEnvelopeMiddleware` as the single error handler; it absorbed the `details` passthrough and the `logError`/Sentry hook the deleted one owned, and answers 500s with `internal` (the `ERROR_CODES` value) rather than the `internal_error` the unreachable copy had drifted to. Every error response now carries the documented `requestId`. `gateway/errorEnvelope.test.ts` (4 tests) asserts it correlates with the `x-request-id` header and that exactly one error handler is mounted; **verified to fail 3/4 against the pre-fix chain** before landing. Full suite 906/906 green — no module depended on the old shape. |
