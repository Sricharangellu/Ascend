...
| Inventory pipeline: Receiving tab | **CLOSED 2026-08-21** | This row claimed a stateful receiving session "doesn't exist". It has since the 2026-08-03 rewrite — `src/modules/purchasing/receiving-sessions.ts` (33 KB) + `receiving-session-routes.ts` implement begin / dock / **scan** / update-line / close / cancel with per-line `accepted_qty`, `held_qty`, `rejected_qty`, lot, expiry, cost override and location. The 2026-08-11 P2P audit corrected the premise and narrowed the row to the real gap — **`POST /receiving/sessions/:id/scan` had no caller anywhere in `web/`**. **PR #230 then built that caller**: `web/app/(protected)/purchasing/receiving/[id]/page.tsx:123` posts to the scan endpoint from a Scan & Receive workspace (ScanBar / ReceivingLinesTable / CostIntelligencePanel / ReviewPanel), reachable in one click from the PO row via `web/lib/receiving.ts`. Verified on `origin/develop`, 2026-08-21. |
...
## Procure-to-Pay gaps (added 2026-08-11, code-verified)

From the P2P design/implementation audit. Full evidence:
`WORK/audits/AUDIT_2026-08-11T042000Z-p2p-design-audit.md`.

**Every row below was re-verified against `origin/develop` on 2026-08-21**, when this branch re-merged `develop`. One row is closed outright by PR #230 and one is narrowed; the rest are confirmed still open, with line numbers refreshed to the current tree.

| Item | Status | What's actually missing |
|---|---|---|
| ~~Receiving desk does not use the canonical barcode resolver~~ | **CLOSED 2026-08-21 by PR #230** | Was the highest-value row here. Two independent fixes landed while this PR sat in draft: the Scan & Receive workspace now calls `POST /receiving/sessions/:id/scan` directly, and `receive-stock/page.tsx` resolves through the canonical `GET /catalog/barcode/:code/pos` — the same endpoint the POS uses, so a case UPC, a vendor UPC and any second each-code all resolve — and **counts** the scan instead of only highlighting a row. The browser no longer runs a weaker duplicate of the resolver. |
| Receiving desk loads one page and filters it client-side | Open | `receive-stock/page.tsx` calls `/purchasing/orders` with no params at `page.tsx:46` (server default `limit=50`) and filters for pending POs in the browser, so a pending PO older than the newest 50 is invisible at the desk. Now trivially fixable: the 2026-08-11 change added `?status=` / `?receiveStatus=` server-side filters. |
| PO document attachment invents a file size | Open | `receive-stock/page.tsx:223` posts `size_bytes: Math.round(Math.random() * 500000 + 50000)`. No file bytes are ever read or uploaded — the UI records a name and a fabricated size. Either upload real bytes or label the control as not implemented. |
| `receive-stock` swallows API failures | Open — **narrowed 2026-08-21** | The `catch { session = null }` around session-begin is gone (PR #230 restructured the scan path). Two bare `catch { /* ignore */ }` blocks remain and still matter: `page.tsx:61` renders a failed pending-PO load as "no pending POs", and `page.tsx:227` drops document-attach errors on the floor. Re-verified on `origin/develop`, 2026-08-21. |
| Purchase requisitions have zero frontend | Open (re-confirmed 2026-08-21) | 9 implemented, tested backend routes (`/requisitions` CRUD + submit/approve/reject/convert). `grep -ril requisition web/` still returns nothing on `origin/develop`. Backend work already paid for, returning no value. |
| Two supplier surfaces | Open | `/purchasing?tab=suppliers` edits name + email; `/vendors/:id` is the full profile with terms, lead time and an on-time/fill-rate scorecard. One table, two forms, two mental models. `/vendors` is the richer one and should become canonical. |
| `exclude_from_po` written, never enforced | Open | The flag is set by the catalog module and read by no code path — a draft, archived, or explicitly excluded product can be added to a purchase order with no error. |
| Dual AP posting (GRNI) | NEEDS-SRI | Receiving posts Dr Inventory / Cr AP unconditionally via an orchestration workflow, while bill entry + three-way match "posts" without touching the ledger. Both paths are individually well-built and they do not reconcile. This is an accounting-policy decision (introduce a GRNI clearing account), not a patch. |
