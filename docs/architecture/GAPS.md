...
| Inventory pipeline: Receiving tab | **CORRECTED 2026-08-11 — the premise is stale** | This row said a stateful receiving session "doesn't exist". It does, and has since the 2026-08-03 procurement-receiving rewrite: `src/modules/purchasing/receiving-sessions.ts` (33 KB) + `receiving-session-routes.ts` implement begin / dock / **scan** / update-line / close / cancel with per-line `accepted_qty`, `held_qty`, `rejected_qty`, lot, expiry, cost override and location, and `receiving-dashboard.ts` aggregates over it. `/purchasing/receiving` renders the dashboard and active sessions. **The real gap is narrower and different: `POST /receiving/sessions/:id/scan` has no caller anywhere in `web/`** (verified by grep, 2026-08-11), while `web/app/(protected)/inventory/receive-stock/page.tsx:114` does its own client-side barcode match against one denormalised field per PO line — so a case UPC does not resolve, and a successful scan only highlights a row instead of entering a quantity. Wiring the existing endpoint is a frontend task, not a decision. Detail: `WORK/audits/AUDIT_2026-08-11T042000Z-p2p-design-audit.md` §3.6. |
...
| Item | Status | What's actually missing |
|---|---|---|
| Receiving desk does not use the canonical barcode resolver | Open — **highest value** | See the corrected Receiving-tab row above. The endpoint exists and is unused; the browser runs a weaker duplicate. |
| Receiving desk loads one page and filters it client-side | Open | `receive-stock/page.tsx` calls `/purchasing/orders` with no params (server default `limit=50`) and filters for pending POs in the browser, so a pending PO older than the newest 50 is invisible at the desk. Now trivially fixable: the 2026-08-11 change added `?status=` / `?receiveStatus=` server-side filters. |
| PO document attachment invents a file size | Open | `receive-stock/page.tsx:149` posts `size_bytes: Math.round(Math.random() * 500000 + 50000)`. No file bytes are ever read or uploaded — the UI records a name and a fabricated size. Either upload real bytes or label the control as not implemented. |
| `receive-stock` masks receiving-session failures | Open | `catch { session = null }` around session-begin silently falls back to the legacy one-shot receive, so the real cause of a failure (e.g. an already-open session) never reaches the user. Its `loadList` also has a bare `catch { /* ignore */ }`, rendering an API failure as "no pending POs". |
| Purchase requisitions have zero frontend | Open (re-confirmed 2026-08-11) | 9 implemented, tested backend routes (`/requisitions` CRUD + submit/approve/reject/convert). `grep -ril requisition web/` returns nothing. Backend work already paid for, returning no value. |
