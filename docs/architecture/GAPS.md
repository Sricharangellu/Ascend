...
| Product/inventory pages: dead routes, broken transfers redirect, duplicate batch/expiry models | **Shipped** (PRs #106–#108 + expiry retirement) — see [PRODUCT_MODULE_REVIEW.md](PRODUCT_MODULE_REVIEW.md) §12 | Dead redirects removed; `/inventory/transfers` → `/inventory?tab=transfers`; `/inventory/expiry` retired (redirect → expiry-pool, no phantom-stock migration); ProductsTab + inventory transfers/orders/returns paginated. Remaining deferred item: shared `DataGrid` (sort/filter/page-size) — not a correctness gap. |
| Dashboard KPI sparklines permanently empty | **Fixed** (2026-07-30) — see [REPORTS_MODULE_REVIEW.md](REPORTS_MODULE_REVIEW.md) finding #5 | `salesSummary` was reading `daily_sales_summary`, which nothing writes. Now dense-fills last 8 UTC days from live completed `orders`. Table left in place for a future CQRS job (NEEDS-SRI / evidence-gated). |
