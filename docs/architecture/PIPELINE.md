...
| `PG_MIGRATION_LOCK_TIMEOUT_MS` | How long a booting instance waits for the migration advisory lock before failing (`0` = wait forever). A `lock_timeout`, deliberately separate from `PG_TX_TIMEOUT_MS`: queuing behind another instance's migrations is correct and must not be charged against the budget bounding your own DDL. Default 5 min | `src/shared/db.ts`, `src/app.ts` | Same | No rotation — tuning value | VERIFIED (code reference + regression test `src/shared/db-tx-timeout.test.ts`) |
