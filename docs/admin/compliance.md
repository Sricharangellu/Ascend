...
2. At checkout, when an age-restricted item is added to the cart, Finder POS shows a full-screen prompt:
...
Finder POS blocks sale of `flavored = true` products in states with active flavor bans (CA, MA, NJ, RI, IL). The list is maintained in `src/lib/complianceRules.ts`. When a restricted product is scanned at checkout and the outlet's state matches a restricted state, a hard block error is shown.
...
For online tobacco/vape sales, PACT Act compliance requires age verification and state tax collection at checkout. Finder POS does not currently automate PACT Act filings — use the compliance export and file manually or via a third-party service.

## Cannabis compliance

Finder POS supports cannabis retail operations as a point-of-sale layer. It does not replace a state-mandated seed-to-sale tracking system (e.g. Metrc). You must integrate Finder POS with your state's required track-and-trace system.
...
Finder POS cannot (and you must handle separately):
- Report to Metrc or other state systems
- Manage dispensary licenses or compliance reporting

## EBT / WIC

EBT (SNAP) and WIC acceptance requires a certified terminal and FNS authorization. Finder POS supports EBT as a tender method via:
...
Every change in Finder POS is logged:
