...
mockHandlers.push(
  ...
  ...(() => {
    ...
    interface InvLine {
      id: string; invoice_id: string; product_id: string | null; upc: string | null;
      sku: string | null; name: string; quantity: number; unit_price_cents: number;
      discount_cents: number; tax_rate_pct: number; line_total_cents: number; sort_order: number;
    }
    interface Inv {
      id: string; invoice_number: string; customer_id: string | null; customer_name: string;
      customer_email: string | null; customer_phone: string | null; billing_address: string | null;
      status: "draft" | "sent" | "partial" | "paid" | "overdue" | "void";
      subtotal_cents: number; tax_cents: number; discount_cents: number; total_cents: number;
      paid_cents: number; due_date: number | null; paid_at: number | null; notes: string | null;
      created_at: number; updated_at: number; lines?: InvLine[];
    }

    const makeLines = (invId: string, items: Array<[string, number, number]>): InvLine[] =>
      items.map(([name, qty, price], i) => ({
        id: `line_${invId}_${i}`, invoice_id: invId, product_id: null, upc: null, sku: null,
        name, quantity: qty, unit_price_cents: price, discount_cents: 0, tax_rate_pct: 8.5,
        line_total_cents: qty * price, sort_order: i,
      }));

    let invoices: Inv[] = [
      { id: "inv_1", invoice_number: "INV-0001", customer_id: "cust_1", customer_name: "Acme Corp",      customer_email: "ap@acme.com",      customer_phone: null, billing_address: "123 Main St", status: "paid",    subtotal_cents: 42000, tax_cents: 3570, discount_cents: 0,    total_cents: 45570, paid_cents: 45570, due_date: BASE - DAY * 5,  paid_at: BASE - DAY * 3, notes: null,                   created_at: BASE - DAY * 15, updated_at: BASE - DAY * 3,  lines: makeLines("inv_1", [["Wireless Keyboard", 3, 8900], ["USB Hub", 2, 6150]]) },
      { id: "inv_2", invoice_number: "INV-0002", customer_id: "cust_2", customer_name: "Bolt Hardware",  customer_email: "orders@bolt.io",    customer_phone: null, billing_address: null,          status: "overdue", subtotal_cents: 18500, tax_cents: 1573, discount_cents: 0,    total_cents: 20073, paid_cents: 0,     due_date: BASE - DAY * 10, paid_at: null,           notes: "Net 30 terms",         created_at: BASE - DAY * 40, updated_at: BASE - DAY * 10, lines: makeLines("inv_2", [["Power Strip (6-outlet)", 5, 3700]]) },
      { id: "inv_3", invoice_number: "INV-0003", customer_id: "cust_3", customer_name: "Summit Events",  customer_email: "finance@summit.co", customer_phone: null, billing_address: null,          status: "partial", subtotal_cents: 75000, tax_cents: 6375, discount_cents: 5000, total_cents: 76375, paid_cents: 40000, due_date: BASE + DAY * 7,  paid_at: null,           notes: "50% deposit received", created_at: BASE - DAY * 5,  updated_at: BASE - DAY * 1,  lines: makeLines("inv_3", [["Event Table (round)", 10, 5500], ["Chair set (6 chairs)", 10, 2000]]) },
      { id: "inv_4", invoice_number: "INV-0004", customer_id: null,     customer_name: "Maria Garcia",   customer_email: "maria@gmail.com",   customer_phone: "555-1234",    billing_address: null, status: "sent",    subtotal_cents: 9800,  tax_cents: 833,  discount_cents: 0,    total_cents: 10633, paid_cents: 0,     due_date: BASE + DAY * 14, paid_at: null,           notes: null,                   created_at: BASE - DAY * 2,  updated_at: BASE - DAY * 2,  lines: makeLines("inv_4", [["Bike Service Tune-Up", 1, 9800]]) },
      { id: "inv_5", invoice_number: "INV-0005", customer_id: "cust_5", customer_name: "Park & Sons LLC",customer_email: "billing@park.biz",  customer_phone: null, billing_address: "88 Oak Ave",  status: "draft",   subtotal_cents: 31200, tax_cents: 2652, discount_cents: 3000, total_cents: 30852, paid_cents: 0,     due_date: BASE + DAY * 30, paid_at: null,           notes: "Pending review",       created_at: BASE - DAY * 0,  updated_at: BASE - DAY * 0,  lines: makeLines("inv_5", [["Industrial Shelving Unit", 4, 7800]]) },
      { id: "inv_6", invoice_number: "INV-0006", customer_id: "cust_6", customer_name: "Sunrise Cafe",   customer_email: "mgr@sunrise.cafe",  customer_phone: null, billing_address: null,          status: "void",    subtotal_cents: 5500,  tax_cents: 468,  discount_cents: 0,    total_cents: 5968,  paid_cents: 0,     due_date: null,            paid_at: null,           notes: "Cancelled order",      created_at: BASE - DAY * 20, updated_at: BASE - DAY * 18, lines: [] },
    ];
    ...
    return [
      ...
      http.get(`${V1}/customer-invoices/lookup-upc`, async ({ request }) => {
        ...
        if (!upc) return HttpResponse.json({ error: { code: "bad_request" } }, { status: 400 });
        const catalog: Record<string, { product_id: string; name: string; price_cents: number; sku: string }> = {
          "012345678905": { product_id: "prod_demo_1", name: "Organic Dark Roast Coffee Beans", price_cents: 1499, sku: "COFFEE-001" },
          "012345678906": { product_id: "prod_demo_2", name: "Wildflower Honey 12oz",            price_cents: 899,  sku: "HONEY-001" },
        };
        const found = catalog[upc];
      ...
      http.get(`${V1}/customer-invoices`, async ({ request }) => {
        ...
        if (q) filtered = filtered.filter(i =>
          i.invoice_number.toLowerCase().includes(q) || i.customer_name.toLowerCase().includes(q),
        );

        const total = filtered.length;
        const page  = filtered.slice(offset, offset + limit).map(({ lines: _l, ...rest }) => rest);
      ...
      http.post(`${V1}/customer-invoices`, async ({ request }) => {
        await lat();
        const body = (await request.json()) as Partial<Inv> & { lines?: Array<Partial<InvLine>> };
        const now = Date.now();
        seq++;
        const id = `inv_${now}`;
        const rawLines: InvLine[] = (body.lines ?? []).map((l, i) => ({
          id: `line_${id}_${i}`, invoice_id: id,
          product_id: l.product_id ?? null, upc: l.upc ?? null, sku: l.sku ?? null,
          name: l.name ?? "Line item", quantity: l.quantity ?? 1,
          unit_price_cents: l.unit_price_cents ?? 0, discount_cents: l.discount_cents ?? 0,
          tax_rate_pct: l.tax_rate_pct ?? 0,
          line_total_cents: (l.quantity ?? 1) * (l.unit_price_cents ?? 0),
          sort_order: i,
        }));
        const subtotal = rawLines.reduce((s, l) => s + l.line_total_cents, 0);
        const tax = Math.round(subtotal * 0.085);
        const inv: Inv = {
          id, invoice_number: `INV-${String(1000 + seq).padStart(4, "0")}`,
          customer_id: body.customer_id ?? null, customer_name: body.customer_name ?? "Walk-in",
          customer_email: body.customer_email ?? null, customer_phone: body.customer_phone ?? null,
          billing_address: body.billing_address ?? null, status: "draft",
          subtotal_cents: subtotal, tax_cents: tax, discount_cents: body.discount_cents ?? 0,
          total_cents: subtotal + tax - (body.discount_cents ?? 0), paid_cents: 0,
          due_date: body.due_date ?? null, paid_at: null, notes: body.notes ?? null,
          created_at: now, updated_at: now, lines: rawLines,
        };
        invoices.push(inv);
        return HttpResponse.json(inv, { status: 201 });
      }),

      http.patch(`${V1}/customer-invoices/:id/status`, async ({ request, params }) => {
        await lat();
        const idx = invoices.findIndex(i => i.id === String(params["id"]));
        if (idx === -1) return HttpResponse.json({ error: { code: "not_found" } }, { status: 404 });
        const body = (await request.json()) as { status: Inv["status"]; paid_cents?: number };
        invoices[idx] = {
          ...invoices[idx]!,
          status: body.status,
          paid_cents: body.paid_cents ?? invoices[idx]!.paid_cents,
          paid_at: body.status === "paid" ? Date.now() : invoices[idx]!.paid_at,
          updated_at: Date.now(),
        };
