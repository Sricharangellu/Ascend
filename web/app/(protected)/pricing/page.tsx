...
const VISIBLE_TAB_KEYS: readonly Tab[] = VISIBLE_TABS.map((t) => t.key);
...
export default function PricingPage() {
  // Default surface is Customer Overrides (the real, backend-backed one).
  // ?tab= deep-links any visible tab (used by the /catalog/price-book redirect).
  const [activeTab, setActiveTab] = useState<Tab>(() => {
    if (typeof window === "undefined") return "customer-overrides";
    const t = new URLSearchParams(window.location.search).get("tab") as Tab | null;
    return t && VISIBLE_TAB_KEYS.includes(t) ? t : "customer-overrides";
  });
      ...
      <div className="mx-auto max-w-[1400px] space-y-6 px-6 py-6">
        {!SHOW_PARTIAL && (
          ...
          >
            ...
            preview-only (no backend yet). Set{" "}
            <code className="font-mono text-xs">NEXT_PUBLIC_SHOW_PARTIAL_PAGES=true</code> to
            explore them. Live discounts stay on Catalog → Discounts.
          </div>
        )}
