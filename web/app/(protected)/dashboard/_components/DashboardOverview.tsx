...
function LiveKpi({ label, value, icon, loading, href }: KpiProps) {
  return (
    <Link
      href={href}
      className="group relative flex flex-col rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-500/30 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
    >
...
export function DashboardOverview({
  ...
  const gpRaw = summary?.kpi?.grossProfitCents;
  const gpDisplay = gpRaw == null ? "—" : formatMoney(gpRaw);
  ...
  const kpis = [
    ...
    { id: "profit", label: "Gross Profit", value: gpDisplay, icon: <IconDollar />, loading: loadingSummary, href: "/reports" },
    { id: "cash", label: "Cash Flow", value: formatMoney(cashFlowCents), icon: <IconActivity />, loading: loadingCash, href: "/reports/cash-movement" },
    ...
    { id: "recs", label: "Recommendations", value: recommendationCount, icon: <IconSparkles />, href: "/reports" },
  ];

  let order = ["revenue", "profit", "cash", "inventory", "pos", "sales", "recs"];

  if (view === "Finance") {
    order = ["cash", "revenue", "profit", "inventory", "sales", "pos", "recs"];
  } else if (industry === "Wholesale" || industry === "Distribution") {
    order = ["pos", "inventory", "revenue", "profit", "sales", "cash", "recs"];
  } else if (industry === "E-commerce") {
    order = ["revenue", "sales", "profit", "cash", "inventory", "pos", "recs"];
  }
