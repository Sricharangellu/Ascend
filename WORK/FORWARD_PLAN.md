...
| **F-18** | 9.6 | OpenAPI contract validation | tooling / contracts | **Medium** | `contracts/openapi.yaml` is generated *from* the code, never checked *against* it | CI check comparing route shapes to the contract | — | 1 d | Medium — may reveal real drift | CI fails on divergence | ✅ **DONE** — `tools/openapi-contract-scan.mjs` (`npm run contract:scan`), gating in CI's guard job. **It did reveal real drift: 9 phantom operations** the contract documented and the backend never served (see F-28 note below for the second class it surfaced). All 9 corrected in the same PR, so the gate is green on arrival. Failure modes proven, not assumed: phantom injection → exit 1; malformed `paths:` → hard error; clean tree → exit 0; undocumented-count growth → warn, exit 0. That last one started out gating and was changed when merging `develop` mid-PR bumped the count on unrelated work — see the audit's §5b |
...
| CI guardrail coverage | ~~12~~ **13** of 16 categories (duplicate-code, dependency advisories, dead-code added 2026-08-04; OpenAPI contract validation added 2026-08-06) |
