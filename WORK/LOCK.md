...
| Files/areas expected | `.github/workflows/ci.yml` (`deploy-production` env block only), `scripts/deploy.sh` (`deploy_frontend` staging layout only — **scope widened mid-claim, see below**), `WORK/LOCK.md`. NOT `src/**`, NOT `web/**`, NOT `uptime.yml`, NOT `deploy_backend`. |
