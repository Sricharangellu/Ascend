...
| Status | RELEASED. Overlap check performed before starting: the only two `ACTIVE` claims below cover `web/**` and `src/modules/payments/**`; this touches neither. |
| Duplicate-work collision (found at push time, not before) | **A second Claude session fixed the same backlog row concurrently** and had already pushed to `fix/migration-lock-statement-timeout` — the branch name this session had independently chosen — and opened **PR #213**. Its claim (`a19e10a`, 19:05Z) landed five minutes after this one started (19:00Z), so neither session could have seen the other's claim; the collision is a real gap in the protocol, not a violation of it. **Nothing of theirs was overwritten**: no force-push, no retarget, no close. This work moved to this session's own designated branch and opened a separate PR that names #213 and states the difference, leaving the choice to a human. |
| Why this one is not redundant with #213 | #213 fixes cause (2) only, by wrapping the blocking lock in `SET LOCAL statement_timeout = 0` and restoring it after. That removes the false failure but makes the wait **unbounded** — a stuck holder now blocks a booting instance forever with no diagnostic, trading a noisy wrong answer for a silent hang. It also leaves cause (1) untouched: all 86 test schemas still serialise on the one global key, so the contention that produced the flake is still there and the suite is no faster. This change bounds the wait, names the lock when it is exhausted, and removes the contention. One idea from #213 does not survive here and is worth recording: it exports `txTimeoutMs()` from `db.ts` to avoid duplicating the 30 s default. Good in its design; unnecessary in this one, which never needs to restore a timeout, so adopting it would add an export with no caller. |
| Blockers | none |
