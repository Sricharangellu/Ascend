...
| Scope change (honest) | **Widened 2026-08-10, on Sri's instruction to continue through the roadmap.** The claim above covers the audit and its two ops fixes. Continuing into the audit's own §17 agent-actionable list added backend source: `src/gateway/{errorEnvelope,accessLog}.ts` (+ their new tests), `src/gateway/index.ts`, `src/app.ts` (middleware chain only), `src/shared/http.ts` (removed the duplicate error handler), `CONTRACTS.md` (shared-kernel line), `src/modules/customer_invoices/routes.ts` (one stale comment), `docs/architecture/GAPS.md`. Checked against the two live `web/**` Cursor Cloud claims first — no overlap; neither lists `src/gateway/**`, `src/app.ts` or `src/shared/http.ts`. |
