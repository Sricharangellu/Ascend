...
export function ErrorsSummaryTab({ onCategoryClick }: Props) {
  ...
  if (loading) {
    return <div className="flex items-center justify-center py-16 text-sm" style={{ color: "var(--color-text-muted)" }}>Loading…</div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className={[
          ...
          style={summary.critical === 0 ? { borderColor: "var(--color-border)", backgroundColor: "var(--color-surface)" } : undefined}
        >
          <p className="text-xs font-medium" style={{ color: "var(--color-text-muted)" }}>Critical</p>
          <p className={[
            "text-3xl font-bold",
            summary.critical > 0 ? "text-red-700" : "",
          ].join(" ")}
            style={summary.critical === 0 ? { color: "var(--color-text-primary)" } : undefined}
          >{summary.critical}</p>
          <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>require immediate action</p>
        </div>
        <div className={[
          "flex flex-col gap-1 rounded-xl border p-4",
          summary.open > 0 ? "border-amber-200 bg-amber-50" : "",
        ].join(" ")}
          style={summary.open === 0 ? { borderColor: "var(--color-border)", backgroundColor: "var(--color-surface)" } : undefined}
        >
          <p className="text-xs font-medium" style={{ color: "var(--color-text-muted)" }}>Open Errors</p>
          <p className={[
            "text-3xl font-bold",
            summary.open > 0 ? "text-amber-700" : "",
          ].join(" ")}
            style={summary.open === 0 ? { color: "var(--color-text-primary)" } : undefined}
          >{summary.open}</p>
          <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>need resolution</p>
        </div>
        <div
          className="flex flex-col gap-1 rounded-xl border p-4"
          style={{ borderColor: "var(--color-border)", backgroundColor: "var(--color-surface)" }}
        >
          <p className="text-xs font-medium" style={{ color: "var(--color-text-muted)" }}>In Review</p>
          <p className="text-3xl font-bold" style={{ color: "var(--color-text-primary)" }}>{summary.in_review}</p>
          <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>being investigated</p>
        </div>
      </div>
            ...
            {activeCategories
              ...
              .map(([key, val]) => (
                <button
                  ...
                  className="flex items-center gap-3 rounded-xl border p-3 text-left transition-all hover:border-indigo-300 hover:shadow-sm"
                  style={{ borderColor: "var(--color-border)", backgroundColor: "var(--color-surface)" }}
                >
                  <span className="text-2xl" aria-hidden="true">{CATEGORY_ICONS[key]}</span>
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium" style={{ color: "var(--color-text-primary)" }}>{val.label}</p>
                    <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>{val.open} open</p>
                  </div>
      ...
      {summary.open === 0 && summary.critical === 0 && (
        <div className="flex flex-col items-center gap-3 py-8" style={{ color: "var(--color-text-muted)" }}>
