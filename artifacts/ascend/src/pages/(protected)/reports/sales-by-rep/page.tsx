...
interface RepItem {
  repId: string; repName: string; orderCount: number; revenueCents: number; avgOrderCents: number;
}
...
function TableSkeleton() {
  return (
    <div aria-busy="true" aria-label="Loading sales by rep data" className="space-y-2 px-1 py-2">
...
export default function SalesByRepPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await apiGet<{ items: RepItem[] }>(`/api/v1/reports/sales-by-rep?range=${range}`);
        if (!cancelled) setItems([...(data.items ?? [])].sort((a, b) => b.revenueCents - a.revenueCents));
      } catch (err) {
        if (!cancelled) setError(err instanceof ApiResponseError ? err.message : "Failed to load sales by rep report.");
      } finally {
  ...
  return (
    <EnterpriseShell active="reports" title="Sales by Rep"
      subtitle={`Sales by representative · Demo Store · ${rangeLabel}`} contentClassName="overflow-y-auto">
      <div className="mx-auto w-full max-w-6xl space-y-5 px-4 py-5 sm:px-6">
        <div className="border-b pb-4" style={{ borderColor: "var(--color-border)" }}>
          <div className="mb-3">
            <h1 className="text-[20px] font-bold tracking-tight" style={{ color: "var(--color-text-primary)" }}>Sales by Representative</h1>
            <p className="mt-1 text-[13px]" style={{ color: "var(--color-text-secondary)" }}>Revenue and order performance broken down by sales rep.</p>
          </div>
          <ReportsSubNav />
        </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-[13px]">
                      ...
                      <tbody className="divide-y divide-[var(--color-table-border)]">
                        {items.map((item, idx) => (
                          <tr key={item.repId} className="transition-colors hover:bg-[var(--color-table-row-hover)]">
                            <td className="py-2.5 pr-4" style={{ color: "var(--color-text-muted)" }}>{idx + 1}</td>
                            <td className="py-2.5 pr-4 font-medium" style={{ color: "var(--color-text-primary)" }}>{item.repName}</td>
                            ...
                            <td className="py-2.5 text-right" style={{ color: "var(--color-text-secondary)" }}>{formatMoney(item.avgOrderCents)}</td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot>
