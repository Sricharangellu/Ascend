...
interface RegisterClosure {
  id: string; register_id: string; register_name: string | null; outlet_name: string | null;
  opened_by: string; opening_float_cents: number; closing_float_cents: number | null;
  counted_cash_cents: number | null; variance_cents: number | null; status: string;
...
export default function RegisterClosuresPage() {
  ...
  const { data: detail, loading: detailLoading } = useQuery(
    ...
    () => selectedId ? apiGet<ClosureDetail>(`/api/v1/reports/register-closures/${selectedId}`) : Promise.resolve(null),
    { staleMs: 30_000 },
  );
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-5 sm:px-6">
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-5">
          ...
          <div className="lg:col-span-2">
            <Card>
              <div className="mb-3 flex items-center justify-between">
                <h2 className="text-[13px] font-semibold" style={{ color: "var(--color-text-primary)" }}>Sessions</h2>
                <span className="text-[11px]" style={{ color: "var(--color-text-secondary)" }}>{sessions.length} total</span>
              </div>
              {loading ? (
                <div className="space-y-2">
                  {[1,2,3,4,5].map(i => <div key={i} className="h-16 animate-skeleton rounded-lg" />)}
                </div>
              ) : sessions.length === 0 ? (
                <p className="py-8 text-center text-[13px]" style={{ color: "var(--color-text-secondary)" }}>No register sessions found.</p>
              ) : (
                    ...
                    <li key={s.id}>
                      <button type="button" onClick={() => setSelectedId(s.id)}
                        className={`w-full rounded-lg px-2 py-3 text-left transition-colors ${
                          selectedId === s.id ? "bg-brand-50" : "hover:bg-[var(--color-table-row-hover)]"
                        }`}>
                        <div className="flex items-center justify-between">
                          <span className="text-[13px] font-medium" style={{ color: "var(--color-text-primary)" }}>
                            {s.register_name ?? s.register_id}
                          </span>
                          <Badge variant={s.status === "open" ? "green" : "gray"} size="sm">{s.status}</Badge>
                        </div>
                        ...
                        {s.variance_cents !== null && s.variance_cents !== 0 && (
                          <div className={`mt-0.5 text-[11px] font-medium ${s.variance_cents < 0 ? "text-[var(--color-danger-text)]" : "text-success-600"}`}>
          ...
          <div className="lg:col-span-3">
            {!selectedId ? (
              <Card>
                <p className="py-12 text-center text-[13px]" style={{ color: "var(--color-text-secondary)" }}>
                  Select a session to view its payment breakdown and cash movements.
                </p>
              </Card>
            ) : detailLoading ? (
              <Card><div className="h-64 animate-skeleton rounded" /></Card>
            ) : detail ? (
              <div className="space-y-4">
                <Card>
                  <h3 className="mb-3 text-[13px] font-semibold" style={{ color: "var(--color-text-primary)" }}>Session Summary</h3>
                  <dl className="grid grid-cols-2 gap-3 text-[13px] sm:grid-cols-3">
                    {[
                      ...
                      <div key={label as string}>
                        <dt className="text-[11px]" style={{ color: "var(--color-text-secondary)" }}>{label}</dt>
                ...
                {detail.paymentBreakdown.length > 0 && (
                  <Card>
                    <h3 className="mb-3 text-[13px] font-semibold" style={{ color: "var(--color-text-primary)" }}>Payment Breakdown</h3>
                    <table className="w-full text-[13px]">
                      ...
                      <tbody className="divide-y divide-[var(--color-table-border)]">
                        {detail.paymentBreakdown.map((p) => (
                          <tr key={p.method}>
                            <td className="py-2 capitalize" style={{ color: "var(--color-text-primary)" }}>{p.method}</td>
                            <td className="py-2 text-right" style={{ color: "var(--color-text-secondary)" }}>{p.count}</td>
                            <td className="py-2 text-right font-medium" style={{ color: "var(--color-text-primary)" }}>{formatMoney(p.total_cents)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                {detail.cashMovements.length > 0 && (
                  <Card>
                    <h3 className="mb-3 text-[13px] font-semibold" style={{ color: "var(--color-text-primary)" }}>Cash Movements</h3>
                    <ul className="divide-y divide-[var(--color-table-border)] text-[13px]">
                      {detail.cashMovements.map((m, i) => (
                        <li key={i} className="flex items-center justify-between py-2">
                          <div>
                            <span className="capitalize" style={{ color: "var(--color-text-primary)" }}>{m.movement_type.replace(/_/g, " ")}</span>
                            {m.reason && <span className="ml-2 text-[11px]" style={{ color: "var(--color-text-secondary)" }}>— {m.reason}</span>}
                          </div>
                          <span className={`font-medium ${m.movement_type === "cash_out" ? "text-[var(--color-danger-text)]" : "text-success-600"}`}>
