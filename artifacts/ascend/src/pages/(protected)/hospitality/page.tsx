    ...
    <EnterpriseShell active="hospitality" title="Rooms" subtitle="Hotel room management and guest folios">
        ...
        <div className="lg:col-span-3 space-y-3">
              ...
              {STATUS_FILTERS.map(s => (
                <button key={s} type="button" onClick={() => setFilterStatus(s)}
                  className={`rounded-full px-3 py-1 text-xs font-medium capitalize transition-colors ${
                    filterStatus === s ? "bg-brand-600 text-white" : "hover:bg-[var(--color-surface-subtle)]"
                  }`}
                  style={filterStatus === s ? {} : { backgroundColor: "var(--color-surface-subtle)", color: "var(--color-text-secondary)" }}>
          ...
          {loading ? (
            <div className="grid grid-cols-3 gap-3">
              {[1,2,3,4,5,6].map(i => <div key={i} className="h-28 animate-skeleton rounded-xl" />)}
            </div>
          ) : visible.length === 0 ? (
            <Card><p className="py-8 text-center text-sm" style={{ color: "var(--color-text-secondary)" }}>No rooms found.</p></Card>
          ) : (
              ...
              {visible.map(room => {
                ...
                return (
                  <button key={room.id} type="button"
                    ...
                    <p className="text-lg font-bold" style={{ color: "var(--color-text-primary)" }}>{room.room_number}</p>
                    <p className="text-xs capitalize" style={{ color: "var(--color-text-secondary)" }}>{room.type}{room.floor ? ` · Fl ${room.floor}` : ""}</p>
                    <div className="mt-2">
                      <Badge variant={style.badge} size="sm">{style.label}</Badge>
                    </div>
                    <p className="mt-1.5 text-xs font-medium" style={{ color: "var(--color-text-secondary)" }}>
          ...
          {!selected ? (
            <Card>
              <div className="py-16 text-center">
                ...
                <p className="mt-2 text-sm" style={{ color: "var(--color-text-secondary)" }}>Select a room to view details</p>
              </div>
            </Card>
          ) : (
            <div className="space-y-3">
              <Card>
                <div className="space-y-3">
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="text-base font-bold" style={{ color: "var(--color-text-primary)" }}>Room {selected.room_number}</h3>
                      <p className="text-sm capitalize" style={{ color: "var(--color-text-secondary)" }}>
                        {selected.type}{selected.floor ? ` · Floor ${selected.floor}` : ""}
                      </p>
                      <p className="text-sm" style={{ color: "var(--color-text-secondary)" }}>{formatMoney(selected.rate_cents)}/night</p>
                    </div>
              ...
              <div>
                ...
                {charges.length === 0 ? (
                  <Card><p className="py-4 text-center text-sm" style={{ color: "var(--color-text-secondary)" }}>No charges.</p></Card>
                ) : (
                  <div className="space-y-1.5">
                    {charges.map(c => (
                      <div key={c.id} className="flex items-center justify-between rounded-lg px-3 py-2"
                        ...
                        <div>
                          <p className="text-sm font-medium" style={{ color: "var(--color-text-primary)" }}>{c.description}</p>
                          <p className="text-xs" style={{ color: "var(--color-text-secondary)" }}>
                            {fmtDateTime(c.posted_at)}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-semibold" style={{ color: "var(--color-text-primary)" }}>{formatMoney(c.amount_cents)}</p>
      ...
      <Modal open={chargeModal} onClose={() => setChargeModal(false)} title={`Post Charge — Room ${selected?.room_number}`}>
        <div className="space-y-3 p-4">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium" style={{ color: "var(--color-text-secondary)" }}>Amount (cents) *</label>
      ...
      <Modal open={addModal} onClose={() => setAddModal(false)} title="Add Room">
        <div className="space-y-3 p-4">
          <div>
            <label className="mb-1 block text-sm font-medium" style={{ color: "var(--color-text-secondary)" }}>Room number *</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium" style={{ color: "var(--color-text-secondary)" }}>Floor</label>
          ...
          <div>
            <label className="mb-1 block text-sm font-medium" style={{ color: "var(--color-text-secondary)" }}>Rate (cents/night)</label>
