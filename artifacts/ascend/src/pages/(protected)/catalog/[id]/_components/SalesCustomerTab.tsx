...
export function SalesCustomerTab({ productId }: { productId: string }) {
  ...
  const SortIcon = ({ col }: { col: keyof SaleRecord }) => (
    <span className="ml-0.5 text-[10px]" style={{ color: "var(--color-text-muted)" }}>
  ...
  if (loading) return (
    <div className="space-y-3">
      {[1,2,3,4].map((i) => <div key={i} className="h-12 animate-skeleton rounded-xl" />)}
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative">
          <svg className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2" style={{ color: "var(--color-text-muted)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <input type="search" value={customerSearch} onChange={(e) => setCustomerSearch(e.target.value)}
            ...
            className="h-8 rounded-lg border pl-8 pr-3 text-[12px] outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500"
        ...
        <div className="flex rounded-lg border p-0.5" style={{ borderColor: "var(--color-border)" }}>
          {(["all","retail","wholesale"] as CustomerType[]).map((t) => (
            <button key={t} type="button" onClick={() => setCustomerType(t)}
              className={`rounded-md px-2.5 py-1 text-[11px] font-medium transition-colors capitalize ${
                customerType === t ? "bg-brand-600 text-white" : ""
              }`}
              style={customerType !== t ? { color: "var(--color-text-secondary)" } : {}}>
              {t === "all" ? "All" : t}
            </button>
          ))}
        </div>
        ...
        <button type="button" onClick={() => setShowFilters((v) => !v)}
          className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-[11px] font-medium transition-colors ${
            activeFilters > 0 ? "border-brand-600 bg-brand-600/5 text-brand-600" : ""
          }`}
          style={activeFilters === 0 ? { borderColor: "var(--color-border)", color: "var(--color-text-secondary)" } : {}}>
        ...
        <div className="ml-auto">
          <button type="button" onClick={() => exportCSV(filtered, productId)}
      ...
      {showFilters && (
        <div className="grid grid-cols-2 gap-3 rounded-xl border p-4 sm:grid-cols-4"
          ...
          {[
            ...
            <div key={label}>
              <label className="mb-1 block text-[11px] font-medium" style={{ color: "var(--color-text-secondary)" }}>{label}</label>
              <select value={value} onChange={(e) => onChange(e.target.value)}
                className="w-full rounded-lg border px-2.5 py-1.5 text-[12px] outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500"
                style={{ borderColor: "var(--color-border)", backgroundColor: "var(--color-surface)", color: "var(--color-text-primary)" }}>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-6">
        {[
          { label: "Unique customers", value: visibleSummary.customers.toString(), extra: "" },
          { label: "Total orders",     value: filtered.length.toString(),          extra: "" },
          { label: "Units sold",       value: visibleSummary.qty.toString(),        extra: "text-blue-600" },
          { label: "Revenue",          value: formatMoney(visibleSummary.revenue),  extra: "text-emerald-700" },
          { label: "Avg order",        value: formatMoney(visibleSummary.avgOrder), extra: "" },
          { label: "Avg margin",       value: `${visibleSummary.avgMargin.toFixed(1)}%`, extra: visibleSummary.avgMargin >= 30 ? "text-emerald-700" : "text-amber-600" },
        ].map(({ label, value, extra }) => (
          <div key={label} className="rounded-xl border shadow-[var(--shadow-sm)] px-3 py-2.5"
            style={{ borderColor: "var(--color-border)", backgroundColor: "var(--color-surface)" }}>
            <p className="text-[11px]" style={{ color: "var(--color-text-muted)" }}>{label}</p>
            <p className={`mt-0.5 text-[15px] font-bold ${extra}`}
      ...
      {filtered.length === 0 ? (
        <div className="rounded-xl border border-dashed py-12 text-center" style={{ borderColor: "var(--color-border)" }}>
          <p className="text-[13px]" style={{ color: "var(--color-text-muted)" }}>No sales match the selected filters.</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-[13px]">
              <thead style={{ backgroundColor: "var(--color-table-header)", borderBottom: "1px solid var(--color-border)" }}>
                <tr>
                  {[
                    ...
                    <th key={col}
                      className="cursor-pointer select-none whitespace-nowrap px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-[0.06em] transition-colors hover:text-[var(--color-text-primary)]"
                      style={{ color: "var(--color-text-secondary)" }}
                      onClick={() => handleSort(col)}>
                      {label}<SortIcon col={col} />
                    </th>
                  ))}
                  <th className="px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-[0.06em]"
                    style={{ color: "var(--color-text-secondary)" }}>Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--color-table-border)]">
                {filtered.map((row) => (
                  <tr key={row.id} className="group transition-colors hover:bg-[var(--color-table-row-hover)]">
                    <td className="px-4 py-3">
                      <div>
                        <button type="button" onClick={() => router.push(`/customers/${row.customer_id}`)}
                          className="text-[13px] font-medium text-brand-600 hover:underline">
                          {row.customer_name}
                        </button>
                        <span className={`ml-2 rounded-full px-1.5 py-0.5 text-[10px] font-semibold ${
                          row.customer_type === "wholesale" ? "bg-violet-100 text-violet-700" : "bg-sky-100 text-sky-700"
                        }`}>
                          {row.customer_type}
                        </span>
                      </div>
                    ...
                    <td className="px-4 py-3">
                      <button type="button" onClick={() => router.push(`/orders/${row.order_id}`)}
                        className="text-[11px] font-medium text-brand-600 hover:underline">
                        {row.order_number}
                      </button>
                    </td>
                    <td className="whitespace-nowrap px-4 py-3 text-[11px]" style={{ color: "var(--color-text-muted)" }}>{fmtDate(row.order_date)}</td>
                    <td className="px-4 py-3 text-[11px]" style={{ color: "var(--color-text-secondary)" }}>{row.outlet}</td>
                    <td className="px-4 py-3 font-semibold" style={{ color: "var(--color-text-primary)" }}>{row.qty_bought}</td>
                    <td className="px-4 py-3 text-[11px]" style={{ color: "var(--color-text-secondary)" }}>{formatMoney(row.unit_price_cents)}</td>
                    ...
                    <td className="px-4 py-3 text-[11px]" style={{ color: "var(--color-text-secondary)" }}>{row.tax_cents > 0 ? formatMoney(row.tax_cents) : "—"}</td>
                    <td className="px-4 py-3 font-semibold" style={{ color: "var(--color-text-primary)" }}>{formatMoney(row.total_cents)}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      {row.returned_qty > 0 ? (
                        <span className="rounded-full bg-red-100 px-2 py-0.5 text-[11px] font-semibold text-red-700">{row.returned_qty}</span>
                      ) : (
                    ...
                    <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button type="button" onClick={() => router.push(`/customers/${row.customer_id}`)} title="View customer"
                          className="rounded p-1 transition-colors hover:bg-[var(--color-surface-subtle)] hover:text-brand-600"
                          style={{ color: "var(--color-text-muted)" }}>
                          <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                          </svg>
                        </button>
                        <button type="button" onClick={() => router.push(`/orders/${row.order_id}`)} title="View order"
                          className="rounded p-1 transition-colors hover:bg-[var(--color-surface-subtle)] hover:text-brand-600"
                          style={{ color: "var(--color-text-muted)" }}>
                          <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        {row.returned_qty > 0 && (
                          <button type="button" onClick={() => router.push(`/returns?order=${row.order_id}`)} title="View return"
                            className="rounded p-1 transition-colors hover:bg-[var(--color-surface-subtle)] hover:text-red-600"
                            style={{ color: "var(--color-text-muted)" }}>
                            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <tfoot className="border-t" style={{ borderColor: "var(--color-border)", backgroundColor: "var(--color-table-header)" }}>
                <tr>
                  <td colSpan={4} className="px-4 py-2.5 text-[11px] font-semibold" style={{ color: "var(--color-text-secondary)" }}>
                    {filtered.length} order{filtered.length !== 1 ? "s" : ""} · {visibleSummary.customers} customer{visibleSummary.customers !== 1 ? "s" : ""}
                  </td>
                  <td className="px-4 py-2.5 text-[13px] font-bold" style={{ color: "var(--color-text-primary)" }}>{visibleSummary.qty}</td>
                  <td colSpan={3} />
                  <td className="px-4 py-2.5 text-[13px] font-bold" style={{ color: "var(--color-text-primary)" }}>{formatMoney(visibleSummary.revenue)}</td>
                  <td className="px-4 py-2.5 text-[11px] font-semibold" style={{ color: "var(--color-text-secondary)" }}>{visibleSummary.avgMargin.toFixed(1)}%</td>
                  <td className="px-4 py-2.5 text-[11px] font-semibold text-red-600">{visibleSummary.returns > 0 ? visibleSummary.returns : "—"}</td>
                  <td />
                </tr>
              </tfoot>
            </table>
