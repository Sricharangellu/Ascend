      ...
      <div className="mx-auto w-full max-w-5xl space-y-4 px-4 py-5 sm:px-6">
            ...
            {(["assets", "contracts"] as const).map(t => (
              <button key={t} type="button" onClick={() => setTab(t)}
                className={`rounded-full px-4 py-1.5 text-sm font-medium capitalize transition-colors ${
                  tab === t ? "bg-brand-600 text-white" : "hover:bg-[var(--color-surface-subtle)]"
                }`}
                style={tab === t ? {} : { backgroundColor: "var(--color-surface-subtle)", color: "var(--color-text-secondary)" }}>
        ...
        {tab === "assets" && (
          loading ? (
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {assets.map(a => (
                <div key={a.id} className="rounded-xl p-4 shadow-sm" style={{ border: "1px solid var(--color-table-border)", backgroundColor: "var(--color-surface)" }}>
        ...
        {tab === "contracts" && (
          loading ? (
            ...
            <div className="space-y-2">
              {contracts.map(c => (
                <div key={c.id} className="flex items-center justify-between rounded-xl px-4 py-3 shadow-sm" style={{ border: "1px solid var(--color-table-border)", backgroundColor: "var(--color-surface)" }}>
                  <div>
