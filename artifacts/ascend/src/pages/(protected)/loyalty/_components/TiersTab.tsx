    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="flex w-full max-w-md flex-col rounded-xl shadow-xl" style={{ backgroundColor: "var(--color-surface)" }} onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: "1px solid var(--color-border)" }}>
          <h2 className="text-base font-semibold" style={{ color: "var(--color-text-primary)" }}>{tier ? "Edit Tier" : "New Tier"}</h2>
        ...
        <form id="tier-form" onSubmit={handleSubmit} className="space-y-4 px-5 py-4">
          ...
          <div>
            <label className={labelCls} style={{ color: "var(--color-text-secondary)" }}>Tier name <span className="text-red-500">*</span></label>
            <input className={inputCls} style={{ borderColor: "var(--color-border)" }} value={name} onChange={e => setName(e.target.value)} required placeholder="e.g. Gold" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelCls} style={{ color: "var(--color-text-secondary)" }}>Level <span className="text-red-500">*</span></label>
              <select className={inputCls} style={{ borderColor: "var(--color-border)" }} value={level} onChange={e => setLevel(e.target.value as LoyaltyTierLevel)}>
                {TIER_LEVELS.map(l => (
            ...
            <div>
              <label className={labelCls} style={{ color: "var(--color-text-secondary)" }}>Discount %</label>
              <input className={inputCls} style={{ borderColor: "var(--color-border)" }} type="number" min="0" max="100" step="0.5"
                value={discountPct} onChange={e => setDiscountPct(e.target.value)} placeholder="0" />
            </div>
          </div>
          <div>
            <label className={labelCls} style={{ color: "var(--color-text-secondary)" }}>Points required to reach this tier</label>
            <input className={inputCls} style={{ borderColor: "var(--color-border)" }} type="number" min="0"
    ...
    <>
      <Card className="overflow-hidden p-0">
        <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: "1px solid var(--color-border)" }}>
          <div>
            <h2 className="text-base font-semibold" style={{ color: "var(--color-text-primary)" }}>Loyalty Tiers</h2>
            <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>{tiers.length} tiers · {totalMembers} total members</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="divide-y divide-[var(--color-table-border)]">
            {tiers.map(tier => (
              <div key={tier.id} className="flex items-center gap-4 px-4 py-4 hover:bg-[var(--color-table-row-hover)]">
                <div className="shrink-0">
                  <Badge variant={TIER_BADGE[tier.level]}>{tier.name}</Badge>
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-3">
                    <span className="text-sm font-medium" style={{ color: "var(--color-text-primary)" }}>{tier.points_required.toLocaleString()} pts required</span>
                    {tier.discount_pct > 0 && (
                  ...
                  {tier.description && <p className="truncate text-sm" style={{ color: "var(--color-text-muted)" }}>{tier.description}</p>}
                </div>
                <div className="shrink-0 text-right">
                  <span className="text-sm font-medium" style={{ color: "var(--color-text-primary)" }}>{tier.member_count}</span>
                  <span className="text-sm" style={{ color: "var(--color-text-muted)" }}> members</span>
                </div>
      ...
      <div className="rounded-lg px-4 py-3 text-sm" style={{ border: "1px solid var(--color-border)", backgroundColor: "var(--color-surface-subtle)", color: "var(--color-text-secondary)" }}>
