    ...
    <EnterpriseShell active="golf-pro-shop" title="Pro Shop" subtitle="Equipment & apparel inventory"
      ...
      <div className="mx-auto w-full max-w-5xl space-y-5 px-4 py-5 sm:px-6">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div className="rounded-xl px-4 py-3" style={{ border: "1px solid var(--color-border)", backgroundColor: "var(--color-surface)" }}>
            <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>SKUs</p>
            <p className="text-xl font-bold" style={{ color: "var(--color-text-primary)" }}>{items.length}</p>
          </div>
          <div className="rounded-xl px-4 py-3" style={{ border: "1px solid var(--color-border)", backgroundColor: "var(--color-surface)" }}>
            <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>Retail Value</p>
            <p className="text-xl font-bold" style={{ color: "var(--color-text-primary)" }}>{formatMoney(totalValue)}</p>
          </div>
          <div className={`rounded-xl border px-4 py-3 ${lowStockCount > 0 ? "border-amber-100 bg-amber-50" : ""}`}
               style={lowStockCount > 0 ? {} : { border: "1px solid var(--color-border)", backgroundColor: "var(--color-surface)" }}>
            <p className={`text-xs ${lowStockCount > 0 ? "text-amber-600" : ""}`}
               style={lowStockCount > 0 ? {} : { color: "var(--color-text-muted)" }}>Low Stock</p>
            <p className={`text-xl font-bold ${lowStockCount > 0 ? "text-amber-700" : ""}`}
               style={lowStockCount > 0 ? {} : { color: "var(--color-text-muted)" }}>{lowStockCount}</p>
          </div>
          <div className="rounded-xl px-4 py-3" style={{ border: "1px solid var(--color-border)", backgroundColor: "var(--color-surface)" }}>
            <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>Out of Stock</p>
            <p className={`text-xl font-bold ${items.filter(i => i.stock_qty === 0).length > 0 ? "text-red-700" : ""}`}
               style={items.filter(i => i.stock_qty === 0).length > 0 ? {} : { color: "var(--color-text-muted)" }}>
            ...
            return (
              <button key={cat}
                      ...
                      className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
                        filterCategory === cat ? "bg-brand-600 text-white" : "hover:bg-[var(--color-surface-subtle)]"
                      }`}
                      style={filterCategory === cat ? {} : { backgroundColor: "var(--color-surface-subtle)", color: "var(--color-text-secondary)" }}>
        ...
        {loading ? (
          <div className="space-y-2">
            {Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-16 animate-skeleton rounded-xl" />)}
          </div>
        ) : visible.length === 0 ? (
          <div className="rounded-xl border border-dashed py-14 text-center" style={{ borderColor: "var(--color-border)" }}>
            <p className="text-sm font-medium" style={{ color: "var(--color-text-secondary)" }}>No items match your filters</p>
            <p className="mt-1 text-xs" style={{ color: "var(--color-text-muted)" }}>Pro shop items are managed from the main Catalog.</p>
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-[var(--color-surface-subtle)]">
                      <td className="px-4 py-3">
                        <p className="font-medium" style={{ color: "var(--color-text-primary)" }}>{item.name}</p>
                        <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>{item.sku}{item.brand ? ` · ${item.brand}` : ""}</p>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-right">
                        <span className={`font-medium ${mg >= 30 ? "text-green-700" : mg > 0 ? "text-amber-700" : ""}`}
                              style={mg > 0 ? {} : { color: "var(--color-text-muted)" }}>
                          {mg > 0 ? `${mg}%` : "—"}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-right">
                        <p className={`font-medium ${item.stock_qty === 0 ? "text-red-700" : item.stock_qty <= item.reorder_pt ? "text-amber-700" : ""}`}
                           style={item.stock_qty === 0 || item.stock_qty <= item.reorder_pt ? {} : { color: "var(--color-text-primary)" }}>
                          {item.stock_qty}
                        </p>
                        <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>min {item.reorder_pt}</p>
                      </td>
                      <td className="px-4 py-3">
